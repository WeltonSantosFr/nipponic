...
import { BookOpen, DoorOpen, Plus, Settings, UserRound } from "lucide-react";
...
export function AppSidebar({
  ...
  const todayNotes = notes.filter((note) => {
    return parseDate(note.updatedAt).toDateString() === todayString;
  });

  const weekNotes = notes.filter((note) => {
    const noteDate = parseDate(note.updatedAt);
